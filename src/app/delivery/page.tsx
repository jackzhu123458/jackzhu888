      ...
      <Dialog open={printPreviewOpen} onOpenChange={setPrintPreviewOpen}>
        <DialogContent className="!max-w-none !p-0 !gap-0 flex flex-col" style={{ width: 'calc(241mm + 40px)', maxHeight: '92vh' }}>
          ...
          <div className="overflow-y-auto flex-1 min-h-0 flex justify-center" style={{ background: '#E5E7EB' }}>
            ...
            <div id="delivery-print-area" className="bg-white shrink-0 my-4" style={{ fontFamily: 'PingFang SC, Microsoft YaHei, SimSun, sans-serif', width: '241mm', minHeight: '139.5mm', padding: '5mm 8mm', boxSizing: 'border-box', fontSize: '22px', lineHeight: '32px', boxShadow: '0 2px 12px rgba(0,0,0,0.15)' }}>
              {/* 抬头区域 — 从系统设置读取公司信息 */}
              <div style={{ textAlign: 'center', marginBottom: '2px' }}>
                <div style={{ fontSize: '28px', fontWeight: 'bold', letterSpacing: '4px' }}>{companyInfo.name || '常州横林新顺电器配件厂'}</div>
                <div style={{ fontSize: '18px', color: '#555', marginTop: '0' }}>
              ...
              <div style={{ textAlign: 'center', fontSize: '26px', fontWeight: 'bold', margin: '2px 0 4px' }}>送 货 单</div>

              {/* 客户信息 + 单号信息 */}
              <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px', fontSize: '22px' }}>
                <div>
                ...
                return pages.map((pageItems, pageIdx) => {
                  ...
                  return (
                    <div key={pageIdx} style={{ pageBreakAfter: isLastPage ? 'avoid' : 'always', pageBreakInside: 'avoid' }}>
                      ...
                      {pageIdx > 0 && (
                        <>
                          <div style={{ textAlign: 'center', marginBottom: '2px' }}>
                            ...
                            <div style={{ fontSize: '26px', fontWeight: 'bold', marginTop: '2px', letterSpacing: '8px' }}>送 货 单</div>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px', fontSize: '20px' }}>
                            <div>
                        ...
                        <table style={{ width: '100%', tableLayout: 'fixed', borderCollapse: 'collapse', border: '1px solid #000' }}>
                          ...
                          <tbody>
                            <tr style={{ background: '#f0f0f0' }}>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>项次</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>订单编号</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>物料编号</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>物料名称</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>单位</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>数量</th>
                              <th style={{ border: '1px solid #000', padding: '2px 6px', fontWeight: 'bold', fontSize: '20px', textAlign: 'center' }}>备注</th>
                              <th rowSpan={totalRows + 1} style={{ border: '1px solid #000', padding: '8px 2px', fontSize: '18px', writingMode: 'vertical-rl', letterSpacing: '1px', lineHeight: '1.4', textAlign: 'center' }}>
                            ...
                            {pageItems.map((item, idx) => {
                              ...
                              return (
                                <tr key={`item-${pageIdx}-${idx}`}>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', textAlign: 'center', fontSize: '20px' }}>{pageIdx * MAX_ROWS + idx + 1}</td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', fontFamily: 'SF Mono, Menlo, Consolas, monospace', fontSize: '20px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {orderNo}
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', fontFamily: 'SF Mono, Menlo, Consolas, monospace', fontSize: '20px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {prod.code || ''}
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {prod.name || ''}{prod.spec ? `/${prod.spec}` : ''}
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', textAlign: 'center', fontSize: '20px' }}>
                                    {translateUnit(prod.unit || '')}
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', textAlign: 'right', fontFamily: 'SF Mono, Menlo, Consolas, monospace', fontSize: '20px' }}>
                                    {item.quantity}
                                  </td>
                                  <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                    {item.remark || ''}
                            ...
                            {Array.from({ length: Math.max(0, totalRows - pageItems.length) }).map((_, i) => (
                              <tr key={`empty-${pageIdx}-${i}`}>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', textAlign: 'center', height: '28px', fontSize: '20px' }}>&nbsp;</td>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px' }}>&nbsp;</td>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px' }}>&nbsp;</td>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px' }}>&nbsp;</td>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px' }}>&nbsp;</td>
                                <td style={{ border: '1px solid #000', padding: '2px 6px', fontSize: '20px' }}>&nbsp;</td>
                      ...
                      {isLastPage && (
                        <>
                          <div style={{ marginTop: '4px', fontSize: '22px' }}>
                            <div>备注：{printData?.remark || ''}</div>
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '12px', fontSize: '22px' }}>
                      ...
                      {/* 非最后一页的分页标记 */}
                      {!isLastPage && <div style={{ fontSize: '18px', color: '#999', textAlign: 'center', marginTop: '8px' }}>第 {pageIdx + 1} 页 / 共 {pages.length} 页（续下页）</div>}
                    </div>
                  );
                });
              })()}
