        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50/50">
              <th className="text-left px-5 py-3 font-medium text-gray-500">入库单号</th>
              <th className="text-left px-5 py-3 font-medium text-gray-500">类型</th>
              <th className="text-left px-5 py-3 font-medium text-gray-500">仓库</th>
              <th className="text-left px-5 py-3 font-medium text-gray-500">状态</th>
              <th className="text-left px-5 py-3 font-medium text-gray-500">明细</th>
          ...
          <tbody>
            {loading ? (
              ...
              <tr><td colSpan={8} className="px-5 py-12 text-center text-gray-400">暂无入库单</td></tr>
            ) : (
              notes.map((note) => (
                <tr key={note.id} className="border-b border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-5 py-3 text-gray-700">{typeLabel[note.type] || note.type}</td>
                  <td className="px-5 py-3 text-gray-700">{note.warehouses?.name || '-'}</td>
                  ...
                  <td className="px-5 py-3 text-gray-600 text-xs">
                    {note.inbound_note_items?.map((item) =>
                      `${item.products?.name || '-'} × ${item.quantity}`
                    ).join('；')}
                  </td>
                  <td className="px-5 py-3 text-gray-500 max-w-[120px] truncate">{note.remark || '-'}</td>
                  <td className="px-5 py-3 text-gray-500 text-xs">{note.created_at?.slice(0, 16)?.replace('T', ' ')}</td>
